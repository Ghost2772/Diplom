...
[Исходный код](https://github.com/Ghost2772/Diplom/tree/portfolio-rework) ·
[Автоматические проверки](https://github.com/Ghost2772/Diplom/blob/portfolio-rework/.github/workflows/ci.yml) ·
[Как запустить](#быстрый-запуск) · [Как посмотреть демо](#маршрут-знакомства) ·
...
Реализовать связанное веб-приложение: посетитель изучает каталог, пользователь
собирает корзину и оформляет учебный заказ, а администратор управляет товарами
и заказами. AI-консультант получает актуальные сведения из каталога и историю
диалога. Проект показывает работу с интерфейсом, API, реляционной моделью данных,
авторизацией, внешним сервисом и контейнерным запуском.

На странице **О проекте** (`/about`) есть обзор возможностей, схема архитектуры,
технические решения и ссылки на их реализацию. Она открывается без авторизации.
Публичный адрес приложения будет добавлен после развёртывания на сервере.

## Маршрут знакомства

После [запуска через Docker](#быстрый-запуск) откройте [приложение](http://localhost:3000).

1. Нажмите **Войти в демо-аккаунт** — регистрация не нужна.
2. Откройте **Каталог**, выберите категорию и страницу товара с характеристиками.
3. Добавьте позицию в **Корзину** и оформите учебный заказ.
4. В **Моих заказах** проверьте состав нового заказа и примеры с другими статусами.
5. В **AI-чате** попробуйте вопрос о каталоге и очистку истории. Ответ модели
   доступен при настроенном GigaChat; остальные разделы работают независимо от него.
6. На странице [О проекте](http://localhost:3000/about) изучите устройство приложения.

Публичная демо-сессия даёт пользовательские права. Администратор использует
собственный аккаунт: описание редактора находится в разделе [Управление товарами](#управление-товарами).
...
- категории и страницы товаров с описаниями, фотографиями и характеристиками;
- поиск и фильтрация по категории и цене на уровне API каталога;
...
| Задача | Решение | Реализация и проверки |
|---|---|---|
| Дать доступ к демо нескольким посетителям | Временный аккаунт для каждого входа, собственные корзина и заказы, проверка срока, очистка истёкших данных | [Сервис демо](weapon-store-backend/app/services/demo.py), [тесты](weapon-store-backend/tests/test_demo_login.py) |
| Сохранить историю при редактировании товара | Название, артикул и цена копируются в позиции заказа | [Модель позиции](weapon-store-backend/app/models/order_item.py), [сценарии редактора](weapon-store-backend/tests/test_admin_products_and_chat.py) |
| Очистить чат, пока модель готовит ответ | Перед сохранением ответа проверяется наличие исходного сообщения | [Сервис AI](weapon-store-backend/app/ai/ai_service.py), [тест очистки из другой вкладки](weapon-store-backend/tests/test_admin_products_and_chat.py) |
| Закрыть административные операции | Общая серверная зависимость проверяет роль для управления данными и загрузки файлов | [Проверка доступа](weapon-store-backend/app/dependencies/auth.py), [тесты ролей](weapon-store-backend/tests/test_user_flow.py) |
| Обновлять структуру и данные воспроизводимо | Миграции Alembic, отдельное заполнение учебного каталога и проверки в CI | [Миграции](weapon-store-backend/alembic/versions), [workflow](.github/workflows/ci.yml) |
...
| ИИ | GigaChat API |
| Инфраструктура | Docker Compose, Nginx, Caddy (HTTPS на VPS), GitHub Actions |

## Архитектура

```mermaid
flowchart TD
    Browser["Браузер / React"] -->|"/api через Nginx"| Backend["FastAPI"]
    Backend -->|"SQLAlchemy Async"| Database[("PostgreSQL")]
    Backend -->|"Контекст каталога и диалога"| AI["GigaChat API"]
    Backend -->|"Загрузка фотографий"| Media["Том product_uploads"]
```
...
Backend проверяет данные через Pydantic, определяет пользователя по JWT и
проверяет административную роль перед изменением общих данных. Модель БД
содержит пользователей, категории, товары, корзины, заказы, их позиции и сообщения.
Ключ GigaChat хранится на сервере; frontend обращается к собственному API чата.
...
- обзор проекта: <http://localhost:3000/about>;
- API: <http://localhost:8000>;
- Swagger: <http://localhost:8000/docs>;
- PostgreSQL: `localhost:5432`.

При стандартном запуске контейнера миграции применяются автоматически.
`SEED_DEMO_DATA=true` дополнительно создаёт демонстрационные категории, товары
и администратора. Учетные данные администратора задаются через
`DEMO_ADMIN_EMAIL` и `DEMO_ADMIN_PASSWORD` в `.env`.
...
Для публичного VPS подготовлены отдельные `docker-compose.prod.yml` и
`.env.production.example`: HTTPS через Caddy, внутренние порты сервисов,
production-настройки и лимиты запросов. Порядок запуска, обновления и резервного
копирования описан в [инструкции по деплою](DEPLOYMENT.md).
...
Этот лимит ограничивает размер демо-данных. В конфигурации для VPS дополнительно
ограничена частота запросов; квота расходов GigaChat настраивается отдельно
в используемом проекте API.
...
Тесты API работают с отдельной временной SQLite-базой. CI дополнительно
поднимает PostgreSQL, применяет миграции, заполняет каталог и проверяет
откат и повторное применение схемы. Вызовы GigaChat в контрактном тесте
заменены HTTP-моком — секретный ключ и оплачиваемые запросы для тестов не нужны.
...
каталога, отдельные демо-сессии и страница презентации проекта. Интерфейс
выполнен в общей тёмной glass-стилистике; основные серверные сценарии покрыты тестами.

Подготовлена отдельная конфигурация публичного сервера с HTTPS и инструкция
по эксплуатации. Следующий шаг — развёртывание и проверка сценариев на VPS.
Возможное дальнейшее развитие: проверка и списание остатков при оформлении,
защита от повторного заказа, поиск и фильтры в пользовательском интерфейсе.
Реальная оплата и доставка не входят в назначение портфолио-проекта.
